...
#将字符串形式的ip地址去掉前面两位192.168，转成整数类型。
def ipToLongshort(ip_str):
    #print map(int,ip_str.split('.'))
    ip_long = 0
    ip_str=ip_str[8:]
    for index,value in enumerate(reversed([int(x) for x in ip_str.split('.')])):
...
np.savetxt(r'D:\Soft\Apache24\htdocs1\v1\test\KMeans\kmeans均值data\np_empty.txt', np_empty, delimiter=',')
